...
#ifndef gk_socket_kqueue_h
#define gk_socket_kqueue_h
...
//static int
//sp_wait_read(int kfd,struct event * e,int sock,void *ud,int max,struct timespec *timeout) {
//    struct kevent ev[max];
//    struct kevent ke;
//    EV_SET(&ke,sock,EVFILT_READ,EV_ENABLE,0,0,ud);
//    int n = kevent(kfd,&ke,0,ev,max,timeout);
//
//    int i;
//    for (i=0;i<n;i++) {
//        e[i].s = ev[i].udata;
